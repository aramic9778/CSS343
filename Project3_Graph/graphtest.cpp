...
void testMSTKruskal() {
  cout << "testMSTKruskal" << endl;
  bool isDirectional = false;

  cout << "test on graph0" << endl;
  Graph g0(isDirectional);
  if (!g0.readFile("graph0.txt")) {
    return;
  }

  globalSS.str("");
  int totalWeight0 = g0.mstKruskal(edgePrinter);
  assert(totalWeight0 == 4);
  assert(globalSS.str() == "[AB 1][BC 3]");

  cout << "test on graph1" << endl;
  Graph g1(isDirectional);
  if (!g1.readFile("graph1.txt")) {
    return;
  }

  globalSS.str("");
  int totalWeight1 = g1.mstKruskal(edgePrinter);
  assert(totalWeight1 == 17);
  assert(globalSS.str() == "[AB 1][BC 1][CD 1][DE 1][EF 1][FG 1][GH 1][XY 10]");

  cout << "test on graph4" << endl;
  Graph g4(isDirectional);
  if (!g4.readFile("graph4.txt")) {
    return;
  }

  globalSS.str("");
  int totalWeight4 = g4.mstKruskal(edgePrinter);
  assert(totalWeight4 == 22);
  assert(globalSS.str() ==
         "[BE 1][DK 1][EG 1][EI 1][IK 1][AE 2][AF 2][CL 3][EJ 3][HL 3][CK 4]");
}
