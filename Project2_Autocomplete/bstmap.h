...
private:
  ...
  void assign(Node *node0, Node *node1);
  ...
  mapped_type &find(Node* curr, const key_type &k);
  
  // helper function for operator []
  // add Node to BST if no such value
  mapped_type &add(const key_type &k, const mapped_type &value);

  // helper function for operator []
  // insert new element to the BST sorted by weight, works recursively
  mapped_type &insert(Node *curr, const key_type &k, const mapped_type &value);
  ...
  void insertLeft(Node *&node, const key_type &k, const mapped_type &value);

  // insert new element to the right of the predecesssor node
  void insertRight(Node *&node, const key_type &k, const mapped_type &value);

  // helper function for getAll(), works recursively
  void inorder_vv(Node *curr, const key_type &k, vector<value_type> &v) const;
  ...
  void inorderHelper(Node *curr, void visit(const value_type &item)) const;

  // helper function for preorder, works recursively
  void preorderHelper(Node *curr, void visit(const value_type &item)) const;

  // helper function for postorder, works recusively
  void postorderHelper(Node *curr, void visit(const value_type &item)) const;

  // helper function for rebalancing BSTree
  // get all nodes in BSTree and store in vector in-order, works recursively
  void allNodesInOrder(Node *curr, vector<Node *> &nodes);

  // helper function for rebalancing BSTree
  // rebuild BSTree using the vector of in-order nodes, works recursively
  void rebuild(const vector<Node *> &nodes, int start, int end);
  ...
  bool isSameNode(const Node *node1, const Node *node2) const;
};
